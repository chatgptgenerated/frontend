...
import { askQuestion, readText, recordSpeech } from "../scripts/MLcallers_tts";
...
export default function Home() {
  ...
  const handleAskQuestion = async () => {
    ...
    try {
      console.log("test");
      const voiceQuestion = await recordSpeech();
      const body_voiceQuestion = voiceQuestion.question;
      const response = await askQuestion(body_voiceQuestion);
  ...
  const test_notebook: string = "A cell is the fundamental unit of life. There are hundreds of millions of distinct types of live cells can be found. A multicellular organism is made up of these cells, whereas a unicellular organism is made up of a single cell.";

  const handleReadText = async () => {
    // API call logic
    try {
      const response = await readText(test_notebook);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white text-gray-800">
      ...
      {/* Main Content */}
      <main className="flex flex-col items-center justify-center text-center mt-30 space-y-6">
      <h1 className="text-5xl font-semibold">Select a notebook:</h1>


        {/* Dropdown */}
        <div>
        <select
            className="border border-black text-3xl px-8 py-4"
        ...
        {/* Listen Button */}
        <button 
        className="px-7 py-4 bg-green-500 text-white text-2xl rounded-lg hover:bg-green-600"
